...
const editItem = () => {
  ...
  let camposObrigatorios = [editCode, editName, editPower, editVoltage];
  let error = false;
...
const listComodos = () => {
  document.getElementById("newRoomLabel").textContent="Cômodo:"
  let url = 'http://127.0.0.1:5001/comodos';
  fetch(url, {
    method: 'get',
  })
  .then(response => response.json())
  .then(data => {
    ...
    data.comodos.forEach(comodo => {
      ...
      option.value = comodo.nome;
      option.textContent = comodo.nome;
      selectElement.appendChild(option);
    });
  })
  .catch(error => {
      console.error('Erro ao buscar os comodos da API:', error);
  });
}
